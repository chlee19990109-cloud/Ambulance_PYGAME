...
* **동적 렌더링 및 미디어 매핑**: 초당 120 FPS(`pygame.time.Clock`)의 부드러운 화면을 재생하고 남은 시간에 따라 인게임 타이머 디자인(초록$\rightarrow$노랑$\rightarrow$빨강)을 변환합니다. `pygame.mixer` 모듈 등으로 별동 효과음과 그래픽 에셋(해상도 맞춤 Transform)을 적용했습니다.
